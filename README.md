...
Latest release is `v4.1.12`.  The jar file - `javafbp-4.1.12.jar` - can be obtained from the Releases folder, or from `build/libs`.   Note: The Maven 'shield' below shows an earlier release. If you click on the Maven shield below, select `Download`, then `jar`.
